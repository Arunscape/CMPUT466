...
step_sizes = np.arange(0.0800, 0.0001, -0.0001)
...
for alpha in step_sizes:
    ...
    data.append({'step size': alpha, 'best decay': best_param, 'best epoch': best_epoch, 'validation performance': best_risk, 'test performance': test_risk})
    ...
    plt.title(f"Loss over 100 epochs, step size={alpha}, decay={best_param}")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.ylim(ymin=0) 
    plt.savefig(f"images/q2c_loss_{alpha}.png")
    plt.close(f) 
    
    f = plt.figure()
    plt.title(f"Risk over 100 epochs, step size={alpha}, decay={best_param}")
    plt.plot(range(MaxIter), risks_val)
    plt.xlabel("Epoch")
    plt.ylabel("Risk")
    plt.ylim(ymin=0) 
    plt.savefig(f"images/q2c_risk_{alpha}.png")
    
    plt.close(f) 

    filenames.append(f"images/q2c_loss_{alpha}.png")
    filenames.append(f"images/q2c_risk_{alpha}.png")
pd.set_option('display.max_columns', None)
df = pd.DataFrame(data, columns=['step size', 'best decay', 'best epoch', 'validation performance', 'test performance'])
print(df)
df.to_csv("partc_results.csv")
for f in filenames:
    print(f"[[./{f}]]")
